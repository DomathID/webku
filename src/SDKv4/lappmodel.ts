...
export class LAppModel extends CubismUserModel {
    ...
    private setupModel(setting: ICubismModelSetting): void {
        ...
        const loadCubismMotion = (): void => {
            ...
            // Find the total number of motions
            ...
            if (LAppDefine.preLoadMotion) {
                LAppDefine.DebugLogEnable && LAppPal.printMessage('perload motion');
                for (let i = 0; i < motionGroupCount; i++) {
    ...
    /**
     * 播放动作音频
     * @param soundFileName 音频文件地址
     * @param soundDelay 音频延迟ms
     */
    public playMotionSound(soundFileName: string, soundDelay: number): void {
    ...
    /**
     * Begin playback of a randomly selected motion.
     * @param group Motion group name
     * @param priority 優先度
     * @param onFinishedMotionHandler Callback function called when the motion playback ends
     * @return Returns the identification number of the started motion.
     * It is used as the argument of isFinished(), which determines whether the individual motion isFinished or not.
     * If you can not start [-1]
     */
    ...
    /**
     * Set the facial expression motion specified by the argument
     *
    ...
    public setExpression(expressionId: string): void {
        ...
        if (this._debugMode) {
            LAppPal.printMessage(`[Live2Dv4] expression: [${expressionId}]`);
        }
        ...
        if (motion != null) {
            ...
            if (this._debugMode) {
                LAppPal.printMessage(`[Live2Dv4] expression[${expressionId}] is null`);
            }
        }
    }
     ...
     *
     * @param group Group name of motion data
     */
